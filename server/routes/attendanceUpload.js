...
router.post('/attendance/upload', async (req, res) => {
  ...
  for (const item of records) {
    if (!item.mobile || !item.financial_timeline) {
      console.warn(`⚠️ Skipping record due to missing keys: ${JSON.stringify(item)}`);
      continue;
    }

    const key = {
      mobile: item.mobile,
      financial_timeline: item.financial_timeline,
    };
    ...
    try {
      ...
      for (const field of Object.keys(item)) {
        if ((field === 'mobile' || field === 'financial_timeline') || JSON.stringify(item[field]) === JSON.stringify(existingItem[field])) continue;
        updateFields[field] = item[field];
        hasChanges = true;
      }
      ...
      console.error(`❌ Error processing ${item.mobile} - ${item.financial_timeline}:`, error);
    }
  }
